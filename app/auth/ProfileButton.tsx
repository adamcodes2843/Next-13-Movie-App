...
const ProfileButton = ({session}) => {
  ...
  return (
    <button type="button" onClick={() => profileClick()} className="">
        {
          session?.user?.image ?
          <Image src={session.user.image} alt="user image and profile dropdown button" className="w-[2.3rem] h-[2.3rem] border-2 rounded-full right-6 md:right-12 top-2 fixed z-50 bg-black hover:border-green-600 border-green-800 active:border-green-200 transition duration-600" width={36} height={36} /> :
