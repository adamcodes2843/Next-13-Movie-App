  ...
  return (
    ...
    <form onSubmit={(e) => handleSubmit(e, formData, reviewId)} className={`${darkMode === true || !session?.user ? 'bg-gray-900 bg-opacity-60' : 'bg-white'}  border-4 border-skin-base md:mt-8 mt-4 2xl:mt-4 flex flex-col justify-center items-center text-center py-4 md:min-w-[38rem] md:w-full md:mx-auto 2xl:w-1/2`}>
        ...
        <input
            ...
            className={`text-center md:text-lg ${darkMode === true || !session?.user ? 'bg-gray-600' : 'bg-gray-300'} border-2 shadow-white rounded mx-auto my-4 xl:w-96 overflow-clip md:w-96 w-9/12 ${!formData.title ? 'border-gray-400 focus:outline-skin-light' : formData.title.length === 100 ? 'focus:outline-red-400 border-skin-light' : 'border-skin-light focus:outline-skin-light'} ${darkMode === true || !session?.user ? 'hover:border-skin-light' : 'hover:border-skin-base'}  focus:border-transparent cursor-pointer focus:ring-0`}
             />
        <div className="w-11/12 flex justify-around items-center mb-1 max-w-[600px] md:text-xl gap-1 hover:cursor-pointer">
        { formData.rating === "" ?
            ratings.map((option) => {
            return (<button type="button" key={option} className={`border-2 rounded-full w-10 h-10 border-gray-400 ${darkMode === true || !session?.user ? 'bg-gray-600 hover:border-skin-light' : 'bg-gray-300 hover:border-skin-base'} `} onClick={() => {setFormData({...formData, rating:option})}}>{option}</button>)
            })
        :
            <button type="button" onClick={() => setFormData({...formData, rating: ""})} className={`border-2 ${darkMode === true || !session?.user ? 'bg-gray-600' : 'bg-gray-300'} border-skin-light rounded-full w-10 h-10 flex justify-center items-center`}>{String(formData.rating)}</button>
        }
        </div>
        <textarea 
