...
export default async function MovieDetail({params}) {
    ...
    return (
        <div className={`mt-16 2xl:mt-32 mx-auto flex flex-col 2xl:flex-row 2xl:justify-center 2xl:items-center 2xl:gap-8 max-w-[1600px]  ${user?.settings?.darkMode === false && 'text-black'}`}>
            <div className={`2xl:bg-opacity-30 2xl:rounded-lg ${user?.settings?.darkMode || !user?.settings ? '2xl:bg-gray-600' : '2xl:bg-white 2xl:border-4 2xl:border-skin-base'} 2xl:p-8 2xl:pb-20`}>
                ...
                <p className={`p-4 bg-gray-900 text-sm md:text-base ${user?.settings?.darkMode === false && 'bg-white'}`}>{res.overview}</p>
            </div>
